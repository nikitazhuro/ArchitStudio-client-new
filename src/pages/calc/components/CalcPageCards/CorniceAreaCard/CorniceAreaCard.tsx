...
import { getCorniceFields, useCalcPageActions } from '../../../../../lib/store/calc';
import { defaultFieldsValues, updateFields } from '../../../helpers';
import { ICorniceAreaCard, IField } from '../../../types';

const CorniceAreaCard: React.FC<ICorniceAreaCard> = ({ className }) => {
  const corniceFields = useTypedSelector(getCorniceFields);
  const { addCorniceFields, removeCalcCard } = useCalcPageActions();

  const [fields, setFields] = useState(corniceFields);
  ...
  const addField = () => {
    setFields((prev) => [
      ...prev,
      { ...defaultFieldsValues.corniceFields[0], id: Date.now() },
    ]);
  };
  ...
  const removeCard = () => {
    fieldsRef.current = defaultFieldsValues.corniceFields;

    removeCalcCard('cornice');
  };
  ...
  useEffect(
    () => () => {
      addCorniceFields(fieldsRef.current);
    },
    [],
  );
  ...
  return (
    <div className={`cardItemWrapper${className}`}>
      <div className='cardItemHeader'>
        <h5>Calculation of the cornice area:</h5>
...
export default CorniceAreaCard;
